...
const Projects = () => {
  ...
  return (
    <section id="portfolio" className="section" style={{ overflow: 'hidden' }}>
              ...
              >
                {currentProjects.map((proj) => (
                  <div key={proj._id} style={{ 
                    ...
                    <h3 style={{ color: '#2196f3', marginBottom: '10px', fontSize: '22px' }}>{proj.title}</h3>
                    <p style={{ 
                    ...
                    <div style={{ marginTop: '15px' }}>
                      <Link to={`/project/${proj._id}`} style={{ color: '#2196f3', textDecoration: 'none', fontWeight: 'bold', marginRight: '20px' }}>Read More →</Link>
                      {proj.link && <a href={proj.link} target="_blank" rel="noreferrer" style={{ color: '#2196f3', textDecoration: 'none', fontWeight: 'bold' }}>External Link <i className="fas fa-external-link-alt"></i></a>}
                    </div>
                  </div>
                ))}
              </motion.div>
