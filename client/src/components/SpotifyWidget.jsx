...
const SpotifyWidget = () => {
  ...
  return (
    <a href={data.songUrl} target="_blank" rel="noreferrer" className="spotify-widget-minimal group block text-left">
      ...
      <div className="spotify-content flex items-center gap-4">
        ...
        <div className="song-info flex flex-col justify-center">
          <p className="song-title text-[#e5e5e5] font-extrabold text-[15px] flex items-center gap-2 group-hover:text-highlight transition-colors leading-tight">
            {data.title}
            <i className="fas fa-external-link-alt text-[10px] text-text-dim group-hover:text-highlight transition-colors"></i>
          </p>
          <p className="song-artist text-[#888888] text-[13px] font-medium leading-relaxed">{data.artist}</p>
        </div>
      </div>
    </a>
  );
